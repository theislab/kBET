multinomial.test <- function (observed, prob, useChisq = FALSE, MonteCarlo = FALSE, ntrial = 1e+05, atOnce = 1e+06, verbose=FALSE) 
{
  ...
  if (MonteCarlo == FALSE) {
    if (useChisq == FALSE) {
      ...
      res <- ExactMultinomialTestChisquare(observed, prob, 
                                           size, groups, numEvents, verbose)
    }
  }
  else {
    if (ntrial < numEvents) {
    ...
    if (useChisq == FALSE) {
      res <- MonteCarloMultinomialTest(observed, prob, 
                                       size, groups, numEvents, ntrial, atOnce, verbose)
    }
    else {
      res <- MonteCarloMultinomialTestChisquare(observed, 
                                                prob, size, groups, numEvents, ntrial, atOnce, verbose)
    }
  }
  invisible(res)
}